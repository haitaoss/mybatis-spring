...
public class SqlSessionTemplate implements SqlSession, DisposableBean {
    ...
    private class SqlSessionInterceptor implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            /**
             * 得到 sqlSession
             * 注：SqlSessionInterceptor 是 SqlSessionTemplate 的成员内部类，所以可以这样子访问外部类的属性
             * */
            SqlSession sqlSession = getSqlSession(SqlSessionTemplate.this.sqlSessionFactory,
                    SqlSessionTemplate.this.executorType, SqlSessionTemplate.this.exceptionTranslator);
            try {
                /**
                 * 使用拿到的 sqlSession 执行 method（就是回到Mybatis的代码了，和Spring没关系了）
                 * */
                Object result = method.invoke(sqlSession, args);
                /**
                 * 在 Spring的事物同步管理器中 没有记录当前这个 sqlSession，那就直接提交事务
                 * */
                ...
                if (sqlSession != null) {
                    /**
                     * 关闭SqlSession，所以下次执行sql，会重新获取SqlSession,即重新通过事务获取新的连接
                     *
                     * 注：不在 spring的事物同步资源中 才会关闭session
                     * */
                    closeSqlSession(sqlSession, SqlSessionTemplate.this.sqlSessionFactory);
                }
            }
